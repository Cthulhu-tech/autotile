import { TILECOUNT_PER_SET } from "../../constant/map";
...
export class TilesetRegistrar {
  ...
  constructor(map: Phaser.Tilemaps.Tilemap, subTile: number) {
    this.map = map; this.subTile = subTile;
  }
  register(keys: string[]): Record<string, Phaser.Tilemaps.Tileset> {
    const out: Record<string, Phaser.Tilemaps.Tileset> = {};
    for (const key of keys) {
      const ts = this.map.addTilesetImage(key, key, this.subTile, this.subTile, 0, 0, this.nextFirstGid) as Phaser.Tilemaps.Tileset;
      out[key] = ts;
